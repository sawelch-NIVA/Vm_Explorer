...
`%notin%` <- Negate(`%in%`)
  ...
  copper_env <- read_excel("R/data/complete_dataset_20250226_1807.xlsx") |>
    ...
    filter(ENVIRON_COMPARTMENT %notin% c("No information/not reported", "Not reported")) |>
    mutate(MEASURED_UNIT = case_match(MEASURED_UNIT,
                                      "mg/kg t.v." ~ "mg/kg dw",
                                      "mg/kg v.v." ~ "mg/kg ww"))

  copper_sites <- read_excel("R/data/complete_dataset_20250226_1807.xlsx", sheet = "Sites_Data")
  ...
  aquatic_data <- copper_clean %>%
    ...
    mutate(plot_group = case_when(
      ENVIRON_COMPARTMENT == "Biota" & MEASURED_UNIT == "mg/kg ww" ~ "Biota/Aquatic (ww)",
      ENVIRON_COMPARTMENT == "Biota" & MEASURED_UNIT == "mg/kg dw" ~ "Biota/Aquatic (dw)",
  ...
  for (i in 1:length(compartments)) {
    ...
    if (nrow(comp_data) > 10) {
      trend_model <- try(loess(log10(mean_value) ~ as.numeric(YearMonth),
                               data = comp_data, span = 0.3), silent = TRUE)
  ...
  norway_fylke_raw <- st_read(dsn = "R/data/Basisdata_0000_Norge_4258_Fylker_GeoJSON.geojson")
  ...
  norway_region <- ne_countries(scale = "medium",
                                returnclass = "sf",
  ...
  norway_region_clipped <- st_crop(norway_region, norway_bbox)
  ...
  freshwater_points <- copper_clean_filtered %>%
    ...
    filter(LONGITUDE >= norway_bbox["xmin"] &
  ...
  faceted_map <- ggplot() +
    # Marine background
    # Country outlines
    geom_sf(data = norway_region_clipped,
            fill = "grey", color = "black", size = 0.5) +

    # All fylke data (both land and coastal)
    geom_sf(data = all_fylke_data,
            aes(fill = mean_value), alpha = 1) +
  ...
  biota_classified <- biota_cleaned %>%
    ...
    mutate(
      ...
      final_group = case_when(
        ...
        grepl("Homarus|Cancer|Carcinus|Nephrops|Palaemon", SAMPLE_SPECIES) ~ "Crustaceans",
  ...
  species_tissue_data <- species_tissue_data %>%
    mutate(
      ...
      final_group = factor(
        final_group,
        levels = c("Fish", "Crustaceans", "Mollusks", "Plants",
                   "Worms", "Insects", "Benthic Organisms", "Other Organisms")
      ),
  ...
  presentation_plot <- ggplot(
    species_tissue_data,
    aes(x = MEASURED_VALUE, y = reorder(species_tissue_with_n, MEASURED_VALUE, median),
        fill = final_group)
  ) +
  ...
  monthly_plot_fylke <- ggplot(seasonal_data_fylke,
                               aes(x = MONTH, y = mean_value, group = water_type,
                                   color = water_type)) +
